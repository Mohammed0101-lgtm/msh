...
int new_process(char** args, int (*func)(char**)) {
    ...
    if (pid == 0) {
        ...
        if (result == ERR_STATUS) {
            fprintf(stderr, RED "Error executing command : %s\n" reset, strerror(errno));
        }
...
int exec_cmd(char** args) {
    static char* builtin_func_list[]     = {"ls",    "cd",    "echo", "rm",    "mv",    "open",
                                            "touch", "cmake", "run",  "mkdir", "clang", "comake",
                                            "build", "pwd",   "head", "grep"};

    static int (*builtin_func[])(char**) = {
      &ls,    &cd,     &echo,  &rm,  &mv,   &shopen, &touch, &cmake, &run, &createDirectory,
      &clang, &comake, &build, &pwd, &head, &grep};
    ...
    // Command not found
    fprintf(stderr, RED "%s : Unsupported command\n" reset, args[0]);
    return NOTSUP_STATUS;
}
